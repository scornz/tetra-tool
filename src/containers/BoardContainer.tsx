...
const BoardContainer = (
  ...
  ref: React.ForwardedRef<HandleObject<BoardEntity | null>>
) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [board, setBoard] = useInstanceHandle<BoardEntity>(ref);
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      if (ctx) {
        const board = new BoardEntity(engine, ctx, 10, 30, 20, true);
        setBoard(board);
